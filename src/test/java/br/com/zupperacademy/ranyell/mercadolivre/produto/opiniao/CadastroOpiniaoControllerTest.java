...
import br.com.zupperacademy.ranyell.mercadolivre.produto.pergunta.PerguntaRequest;
...
class CadastroOpiniaoControllerTest {
    ...
    @Test
    void deveCadastrarUmaNovaOpiniao() throws Exception {
        OpiniaoRequest request = new OpiniaoRequest("É um bom Produto", 4, "Muito Bom");
        var json = jsonMapper.writeValueAsString(request);
        //ação e validação
        mockMvc.perform(post("/produtos/" + produto.getId() + "/opinioes")
    ...
    @Test
    void naoDeveCadastrarUmaOpniaoQuandoOProdutoNaoExistir() throws Exception {
        OpiniaoRequest request = new OpiniaoRequest("É um bom Produto", 4, "Muito Bom");
        var json = jsonMapper.writeValueAsString(request);
        //ação e validação
        mockMvc.perform(post("/produtos/" + Long.MAX_VALUE + "/opinioes")
                .contentType(APPLICATION_JSON)
                .header("Authorization", "Bearer " + token)
                .content(json))
