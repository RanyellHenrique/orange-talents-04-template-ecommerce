...
class CadastroPerguntaControllerTest {
    ...
    @Test
    void deveCadastrarUmaNovaPergunta() throws Exception {
        var request = new PerguntaRequest("Qual o valor do frete?");
        var json = jsonMapper.writeValueAsString(request);

        mockMvc.perform(post("/produtos/" + produto.getId() + "/perguntas")
    ...
    @Test
    void naoDeveCadastrarUmaNovaPerguntaQuandoOprodutoNaoexistir() throws Exception {
        var request = new PerguntaRequest("Qual o valor do frete?");
        var json = jsonMapper.writeValueAsString(request);

        mockMvc.perform(post("/produtos/" + Long.MAX_VALUE + "/perguntas")
                .contentType(APPLICATION_JSON)
                .header("Authorization", "Bearer " + token)
                .content(json))
